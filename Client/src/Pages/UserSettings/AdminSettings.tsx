...
export default function AdminSetting() {
  ...
  const handleDelete = async () => {
    try {
      const data = await axios.delete(`${url}/dashboard/user`, {
        headers: {
  ...
  const validation = (input:any) => {
    ...
    if (
      !input.firstName ||
      !/^(?:[A-Z][a-zA-Z]*)(?: [A-Z][a-zA-Z]*){0,2}$/.test(input.firstName)
    ) {
      errors.firstName =
    ...
    if (
      !input.lastName ||
      !/^(?:[A-Z][a-zA-Z]*)(?:-[A-Z][a-zA-Z]*){0,1}$/.test(input.lastName)
    ) {
      errors.lastName =
        "Debe tener un apellido válido con la primera letra mayúscula. Permite compuestos separados por un guión (-)";
    }

    if (!input.password || input.password.length < 6) {
      errors.password =
        "Debe tener una contraseña válida con al menos 6 caracteres.";
    }

    if (!input.phoneNumber || !/^\d{10}$/.test(input.phoneNumber)) {
      errors.phoneNumber =
        "Debe tener un número de teléfono válido de 10 dígitos.";
    }
              ...
              <input
                ...
                onChange={(event) =>
                  setInput({
                    ...
                    isCheckedValue: event.target.checked ? "admin" : "normal",
                  })
                }
